...
async def test_run_pvt_healthcheck(agent):
    ...
    assert report_path.endswith("_pvt_report.html")
    ...
    # Cleanup
    import shutil
    shutil.rmtree(agent.browser_agent.output_dir, ignore_errors=True)
...
async def test_triage_alert(agent):
    ...
    assert report_path.endswith("_triage_report.html")
